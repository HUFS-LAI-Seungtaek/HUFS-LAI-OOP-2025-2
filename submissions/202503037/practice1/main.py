...
class Country:
    ...
    def __add__(self, other):
        return country(self.name, self.gold+other.gold, self.silver+other.silver, self.bronze+other.bronze)

    # Comparison overriding → 금 > 은 > 동 순서로 비교
    def __lt__(self, other: "Country") -> bool:
        if self.gold<other.gold:
            return True
        elif self.gold==other.gold and self.silver<other.silver:
            return True
        elif self.gold==other.gold and self.silver==other.silver and self.bronze<other.bronze:
            return True
        return False
    #AI도움
